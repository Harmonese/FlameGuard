...
class TranscriberA:
    ...
    # New MPC path: Transcriber A no longer owns the preheater dynamics.
    # It only extracts state information already maintained by the forward proxy.
    def build_mpc_context(self, *, preheater_state, feed: FeedObservation, resource: ResourceBoundary | None = None) -> dict:
        resource = resource or ResourceBoundary(
            T_stack_cap_C=self.cfg.resource_T_stack_default_C,
            v_stack_cap_mps=self.cfg.resource_v_stack_default_mps,
        )
        return {
            "time_s": feed.time_s,
            "preheater_state": preheater_state,
            "feed": feed,
            "resource": resource,
        }
