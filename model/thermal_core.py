...
from .model_types import Config, DynamicTargetBand, EquivalentProperties, ResourceBoundary
...
def validate_dynamic_target_band(band: DynamicTargetBand) -> DynamicTargetBand:
    if band.omega_min >= band.omega_max:
        raise ValueError("Dynamic target band must satisfy omega_min < omega_max.")
    return band
...
def tau_target(Tm: float, props: EquivalentProperties, omega_target: float, cfg: Config) -> float:
    ...
    if denom < 0:
        raise ValueError("omega0 < 0.20, current tau-scaling model is not applicable.")
    if abs(denom) <= 1e-12:
        if abs(props.omega0 - omega_target) <= 1e-12:
            return 0.0
        raise ValueError("omega0 == 0.20 only allows omega_target == 0.20.")
